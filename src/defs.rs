...
pub const HARD_DOCS: &[(&str, &str)] = &[
    ...
    ("colors", "styles\n自定义颜色"),
    ("style", "style\n指定匹配器的颜色风格"),
    ("childrenStyle", "类似 style, 但不包含 start 和 end 匹配的区域, 结果覆盖 style"),
    ("missingStyle", "使用 childrenSyntax 时目标语法不存在或未能匹配时, 使用的颜色风格"),
    ("childrenSyntax", "将 start-end 之间的文本使用另一个语法文件来高亮, 与 contains 属性互斥\n支持复杂的动态匹配语法文件, 参考教程 '语法文件开发'"),
    ("color", "style\n指定匹配器的颜色风格"),
    ("comment", "comment\n定义语言注释, 主要用于切换注释功能"),
